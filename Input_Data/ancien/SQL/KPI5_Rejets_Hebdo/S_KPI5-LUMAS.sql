...
-- ============================================================================
-- KPI5 - Plus ancien rejet pour origine = 'LUMAS'
-- Decline de la requete globale S_KPI5-Min_Max_DSI_rejets.sql.
-- Le pre-filtre PSEPCODORI = 'LUMAS' est applique au plus tot (CTE 'ori')
...
-- Code rejet : dernier rejet du dossier (sur dossiers 'LUMAS' uniquement)
rj AS (
...
FROM (
    SELECT
        ori.PSEPCODORI,
        'LUMAS — Luminess'                                          AS LIBELLE_ORIGINE,
        das.PSASBOITRT,
        rj.PSRJCODREJ,
